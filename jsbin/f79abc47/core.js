(function() {
  ...
  function normalize(values, style) {
    ...
    for (;0 == normalized && subIdx < subCount;subIdx++) {
      ...
      do {
        ...
        /** @type {number} */
        normalized = ((0 == a[1].length ? 0 : parseInt(a[1], 10)) < (0 == b[1].length ? 0 : parseInt(b[1], 10)) ? -1 : (0 == a[1].length ? 0 : parseInt(a[1], 10)) > (0 == b[1].length ? 0 : parseInt(b[1], 10)) ? 1 : 0) || (((0 == a[2].length) < (0 == b[2].length) ? -1 : (0 == a[2].length) > (0 == b[2].length) ? 1 : 0) || (a[2] < b[2] ? -1 : a[2] > b[2] ? 1 : 
        0));
      } while (0 == normalized);
    }
    return normalized;
  ...
  function scroll(pdataCur) {
    ...
    if (pdataCur.getBoundingClientRect) {
      ...
      if (self.getBoxObjectFor && !isDead) {
        ...
        /** @type {Object} */
        el = pdataCur;
        for (;(el = getOffsetParent(el)) && (el != self.body && el != udataCur);) {
   ...
   * @return {undefined}
   */
  function onResize(element, h, options) {
    var settings = {};
    if (0 === h) {
      /** @type {number} */
      settings.action_like_video = 1;
    } else {
      if (1 === h) {
        /** @type {number} */
        settings.action_dislike_video = 1;
      } else {
        /** @type {number} */
        settings.action_indifferent_video = 1;
      }
    }
    /** @type {(Function|string)} */
    settings.video_id = element;
    settings.plid = options.dg;
    element = {
      screen : key({
        h : screen.height,
        w : screen.width,
      ...
      session_token : reset("watch_actions_ajax")
    };
    if (options.Ld) {
      element.station_id = options.Ld;
    }
    i("/watch_actions_ajax", {
      format : "XML",
      method : "POST",
      n : settings,
      B : element,
   ...
   */
  function getDeps() {
    var v = require("watch-like-unlike");
    return same(v, "liked") ? 0 : same(v, "unliked") ? 1 : 2;
  }
  /**
   * @param {boolean} val
   * @return {undefined}
   */
  function tester(val) {
    var value = require("watch-like-unlike");
    value = expect(get(target.getInstance()), value);
    slice(value, function(data) {
      if (!same(data, "yt-always-enabled")) {
        /** @type {boolean} */
        data.disabled = !val;
      }
    });
  ...
  function exec(deepDataAndEvents) {
    if (!push("LOGGED_IN")) {
      /** @type {string} */
      var found = "rating-enabled=" + push("ALLOW_RATINGS");
      if (0 === deepDataAndEvents) {
        clear("logged-out-like", found, element);
      } else {
        if (1 === deepDataAndEvents) {
          clear("logged-out-dislike", found, element);
        }
      }
    }
    if (push("ALLOW_RATINGS")) {
      if (found = !getEnumerableProperties()) {
        if (push("YPC_CAN_RATE_VIDEO")) {
          /** @type {boolean} */
          found = t;
        } else {
          resize("watch-actions-rental-required");
          /** @type {boolean} */
          found = TRUE;
        }
        /** @type {boolean} */
      ...
      if (found) {
        if (push("YPC_SHOW_VPPA_CONFIRM_RATING") && !confirm(replace("VPPA_CONFIRM"))) {
          _init();
        } else {
          if (2 !== deepDataAndEvents) {
            configure();
          }
          tester(t);
          deactivate(deepDataAndEvents);
          found = {
            dg : push("PLAYBACK_ID"),
            Ld : push("STATION_ID"),
            /**
             ...
             */
            i : function(value, obj) {
              var c = require("watch-like-unlike");
              if (0 === deepDataAndEvents) {
                addClass(c, "unliked", "liked");
              } else {
                if (1 === deepDataAndEvents) {
                  addClass(c, "liked", "unliked");
                } else {
                  removeClass(c, "liked", "unliked");
                }
              }
              if (2 !== deepDataAndEvents) {
                parseOptions(obj.html_content);
                pass(require("watch-actions-ajax"));
              }
              deactivate(deepDataAndEvents);
            },
            /** @type {function (?, ?): undefined} */
          ...
          var data = push("VIDEO_ID");
          found = found || {};
          onError("watch_actions_ajax", bind(onResize, global, data, deepDataAndEvents, found), found.onError);
        }
      }
    } else {
      resize("watch-actions-ratings-disabled");
    }
  }
  /**
   * @param {number} deepDataAndEvents
   ...
   */
  function p(d, v) {
    var data = d || require("watch-flag");
    if (onProgress(data) && !getEnumerableProperties()) {
      configure();
      /** @type {number} */
      var seconds = 0;
      var youtubeObject = forEach();
      if (youtubeObject) {
        if (youtubeObject.pauseVideo) {
          youtubeObject.pauseVideo();
          seconds = youtubeObject.getCurrentTime();
        }
      }
      /** @type {number} */
      seconds = Math.floor(seconds);
      /** @type {number} */
      youtubeObject = Math.floor(seconds / 60);
      seconds = {
        action_get_flag_video_component : 1,
        video_id : push("VIDEO_ID"),
        t_mins : youtubeObject,
        t_secs : seconds - 60 * youtubeObject
      };
      if (v) {
        /** @type {number} */
        seconds.from_dislike = 1;
      }
      i("/watch_ajax", {
        format : "XML",
        method : "GET",
        ...
        i : function(value, obj) {
          parseOptions(obj.html_content);
          var Block = require("flag-video-form");
          var nodes = require("flag-video-menu");
          if (Block) {
            if (nodes) {
              new registerEvents(Block, nodes, function() {
                _init();
              }, function() {
                /** @type {boolean} */
                data.disabled = TRUE;
  ...
  function go() {
    if (getTimeout()) {
      var result = html("watch-actions-share");
      result = callback("yt-uix-button-toggled", result);
      result = equal(result, "video-id");
    } else {
  ...
  function computed() {
    ...
    var val = value;
    if (getTimeout()) {
      if (require("share-with-playlist").checked) {
        val = valid ? debug().Na() : value;
      }
    }
    return val;
  }
   ...
   * @return {undefined}
   */
  function reload(event, deepDataAndEvents) {
    if (onProgress(event)) {
      var tempCtx = go();
      var dataTable = computed();
      draw(tempCtx, dataTable, deepDataAndEvents);
    }
  }
  /**
   * @param {number} ctx
   ...
   * @return {undefined}
   */
  function draw(ctx, dataTable, deepDataAndEvents) {
    var child = html("watch-actions-share-panel");
    if (ctx != currentContext || dataTable != retValue) {
      /** @type {number} */
      currentContext = ctx;
      retValue = dataTable;
      resize("watch-actions-share");
      call("watch-actions-share-loading");
      extend("watch-actions-player-sidebar-height-changed");
      split("watch-actions-share-panel");
      transformed = new X(child, ctx, dataTable, function() {
        split("watch-actions-share-loading");
        resize("watch-actions-share-panel");
      });
    } else {
      resize("watch-actions-share");
      resize("watch-actions-share-panel");
    ...
    if (deepDataAndEvents) {
      clear("shareOpenedFromFlash", element, element);
    } else {
      clear("shareOpenedFromActionBar", element, element);
    }
    var svg = forEach();
    var f = play(function() {
      var results = same(require("watch-share"), "active");
      if (!svg || !results) {
        readdir(f);
      } else {
        /** @type {Array} */
        results = [];
        /** @type {number} */
        var data = Math.floor(svg.getCurrentTime());
        for (;0 < data;) {
          results.unshift(data % 60);
          /** @type {number} */
          data = Math.floor(data / 60);
        }
        for (;2 > results.length;) {
          results.unshift(0);
        }
        results = _keys(results, function(dataAndEvents, deepDataAndEvents) {
          return 0 < deepDataAndEvents && 10 > dataAndEvents ? "0" + dataAndEvents : dataAndEvents;
        });
        results = results.join(":");
        data = transformed;
        if (data.aa) {
          if (!same(data.aa, "focused") && !data.X) {
            data.aa.value = results;
            data.Qb();
          }
        }
      }
  ...
  function onProgress(data) {
    /** @type {boolean} */
    var onProgress = !same(data, "active");
    _init();
    if (onProgress) {
      end(data, "active");
  ...
  function parseOptions(str) {
    html("watch-actions-ajax").innerHTML = str;
    resize("watch-actions-ajax");
  }
  /**
   * @param {?} value
   * @param {?} obj
   * @return {undefined}
   */
  function setContent(value, obj) {
    var xhtml = obj && (obj.error_message || obj.errors && obj.errors[0]);
    if (!xhtml) {
      xhtml = replace("WATCH_ERROR_MESSAGE");
    }
    html("watch-error-string").innerHTML = xhtml;
    resize("watch-actions-error");
  }
  /**
   * @param {string} name
   * @return {?}
  ...
  function resize(value) {
    var node = html("watch-actions-area-container");
    value = html(value);
    var parser = html("watch-actions-loading");
    var element = html("watch-actions-area");
    if (!getSize(node)) {
      /** @type {string} */
    ...
    split(parser);
    call(value);
    end(node, "transitioning");
    /** @type {string} */
    var newHeight = element.offsetHeight + "px";
    attr(function() {
      /** @type {string} */
    ...
    attr(function() {
      if (node.style.height == newHeight) {
        removeClass(node, "transitioning");
        /** @type {string} */
   ...
   */
  function _init() {
    var name = html("watch-actions-area-container");
    split(name);
    name = expect("watch-actions-panel", name);
    slice(name, function(tag) {
      split(tag);
    });
    extend("watch-actions-player-sidebar-height-changed");
    /** @type {string} */
    html("watch-actions-ajax").innerHTML = "";
    name = html("watch-actions");
    var result = html("watch-subactions");
    name = add("button", value, name);
    if (result) {
      result = add("button", value, result);
      deepMatches(deepEqual(name), deepEqual(result));
    }
    slice(name, function(obj) {
  ...
  function getEnumerableProperties() {
    return!push("LOGGED_IN") ? (resize("watch-actions-logged-out"), TRUE) : t;
  }
  /**
   * @param {Object} f
   * @return {undefined}
  ...
  function prepare() {
    if (push("WATCH7_ENABLED")) {
      ...
      reload(require("watch-share"), TRUE);
    }
  }
  /**
   * @param {?} time
  ...
  /** @type {null} */
  var className = value;
  run("yt.www.watch.ads.setAdId", function(_$) {
    window.ad_debug = _$;
  });
  run("yt.www.watch.ads.showForcedMpu", function(y) {
    /** @type {string} */
    require("ad300x250").innerHTML = ['<iframe src="', y, '" name="ifr_300x250ad" id="ifr_300x250ad" width="300" height="250" marginwidth="0" marginheight="0" hspace="0" vspace="0" frameborder="0" scrolling="no"></iframe>'].join("");
    call("ad300x250");
  });
  run("yt.www.watch.ads.createGutSlot", function(classNames) {
    if (googletag) {
      replacement = googletag.defineSlot(classNames, [[300, 250], [300, 60]], "yt-gut-content");
      replacement.set("ad_type", "flash");
      replacement.addService(googletag.companionAds());
      googletag.enableServices();
      classNames = replacement.getSizes();
      returnVal = classNames[0];
      className = classNames[1];
    }
  });
  run("yt.www.watch.ads.handleMoveGutCompanion", function(txt) {
    txt = txt ? require("instream_google_companion_ad_div") : require("google_companion_ad_div");
    var c = require("yt-gut-content");
    if (c) {
      /** @type {string} */
      c.innerHTML = "";
    } else {
  ...
  run("yt.www.watch.ads.setGutSlotSizes", function(dataAndEvents, deepDataAndEvents) {
    if (replacement) {
      var act = replacement.getSizes();
      matchArray(act);
      if (deepDataAndEvents) {
        act.push(returnVal);
      }
  ...
  run("yt.www.watch.ads.handleSetCompanion", generate);
  run("yt.www.watch.ads.handleSetCompanionForInstream", wrap);
  run("yt.www.watch.ads.handleSetCompanionForLongform", failure);
  run("yt.www.watch.ads.handleSetCompanionForFreewheel", initialize);
  run("yt.www.watch.ads.handleHideCompanion", top);
  run("yt.www.watch.ads.handleHideCompanionForInstream", attributeName);
  run("yt.www.watch.ads.disablePopoutButton", cmd);
  run("yt.www.watch.ads.enablePopoutButton", diff);
  run("yt.www.watch.ads.handleCloseMpuCompanion", parentName);
  run("yt.www.watch.ads.updatePopoutAds", ok);
  run("yt.www.watch.ads.handleSetAfvCompanionVars", rvar);
  run("yt.www.watch.ads.handleShowAfvCompanionAdDiv", lookup);
  run("yt.www.watch.ads.handleHideAfvInstreamCompanionAdDiv", translate);
  run("yt.www.watch.ads.AD_LOAD_TOPIC", "watchads-adLoad");
  run("yt.www.ads.pyv.pyvWatchAfcWithPpvCallback", function(value) {
    resolve(value);
    if (0 < value.length) {
      value = expect("related-video-featured");
      if (2 == value.length) {
        if (same(value[0], "related-video-featured-booster")) {
          call(value[1]);
        } else {
          call(value[0]);
        }
      } else {
        slice(value, function(isXML) {
          call(isXML);
        });
      }
    }
  });
  run("yt.www.ads.pyv.pyvWatchAfcCallback", resolve);
  run("yt.www.ads.pyv.pyvSearchRhsAfcCallback", function(str) {
    var failuresLink = require("pyv-ads");
    if (0 != str.length) {
      if (failuresLink) {
        lines(str, failuresLink, "search_rhs");
      }
    }
  });
  run("yt.www.ads.pyv.pyvSearchTopAfcCallback", function(str) {
    var val = require("pyv-top-ads");
    if (!str.length || !val) {
      parenthesize(val);
    } else {
      lines(str, val, "search_top");
    }
  });
  run("yt.www.ads.pyv.pyvHomeAfcCallback", function(plan) {
    var oldconfig;
    var p;
    if (push("PYV_FEED")) {
      /** @type {string} */
    ...
    if (!result || 0 == plan.length) {
      ...
      invoke(oldconfig, plan[0], result, function() {
        /** @type {(HTMLElement|null)} */
        var item = document.getElementById("pyv-large-dest-url-image");
        /** @type {(HTMLElement|null)} */
        var el = document.getElementById("pyv-feed-item-headline-dest-url");
        /** @type {(HTMLElement|null)} */
        var parent = document.getElementById("pyv-feed-item-thumb-dest-url");
        /** @type {(HTMLElement|null)} */
        var demoLink = document.getElementById("pyv-large-dest-url");
        if (item) {
          item.setAttribute("href", old);
        }
        if (el) {
        ...
        if (parent) {
          parent.setAttribute("href", old);
        }
        if (demoLink) {
          demoLink.setAttribute("href", old);
        }
        pass(result);
      });
    }
  });
  run("yt.www.ads.pyv.showHomepagePpvAd", tmpl);
  run("yt.www.ads.pyv.showPpvOnWatch", reduce);
  run("yt.www.ads.pyv.loadPyvIframe", loaded);
  run("yt.www.ads.pyv.loadPyvAfsScript", function(dataAndEvents) {
    onload("/pyv_afs_ads?query=" + dataAndEvents, element);
  });
  run("yt.www.ads.pyv.hijackPyvSlotForCompanion", function(video_id, walkers) {
    if (window.pyv_backfill_vars || window.pyv_backfill_with_ppv) {
      /** @type {boolean} */
      window.pyv_backfill_vars = window.pyv_backfill_with_ppv = t;
      request("/pyv?" + key({
        video_id : video_id,
        ...
        onComplete : function(value) {
          if (value) {
            convert(value, walkers);
          }
        }
      });
    }
  });
  run("yt.www.ads.pyv.pyvSlotCompanionCallback", convert);
  run("yt.www.ads.pyv.backfillCompanionWithPyv", function() {
    if (window.pyv_backfill_vars) {
      loaded(window.pyv_backfill_vars);
    } else {
      if (window.pyv_backfill_with_ppv) {
        reduce();
      }
    }
    /** @type {boolean} */
  ...
  run("yt.www.ads.pyv.updatePyvSlotCompanionLabel", function(i) {
    var e = require("pyv-slot-companion-status-label");
    if (e) {
      e.innerText = i ? Right : word;
    }
  });
                        ...
                        Panda.document.utils.findElement(Panda.document.utils.findElement(Panda.document.utils.findElement(Panda.document.utils.findElement(obj, 'contents'), 'tabs'), 'content'), 'contents').contents.forEach(video => {
                            ...
                            if (video.richItemRenderer) {
                                ...
                                if (obj.header.pageHeaderRenderer) {
                                    videoauthor = Panda.document.utils.createStringFromObject(Panda.document.utils.findElement(Panda.document.utils.findElement(obj, 'header'), 'pageTitle'))
                                } else {
                                    videoauthor = Panda.document.utils.createStringFromObject(Panda.document.utils.findElement(Panda.document.utils.findElement(obj, 'header'), 'title'))
                                }
                                var videoviews = ``
                                if (Panda.document.utils.findElement(video, 'viewCountText')) {
                                  videoviews = Panda.document.utils.createStringFromObject(Panda.document.utils.findElement(video, 'viewCountText'))
                                }

var videotime = ``
if (Panda.document.utils.findElement(video, 'lengthText')) {
videotime = Panda.document.utils.createStringFromObject(Panda.document.utils.findElement(Panda.document.utils.findElement(video, 'lengthText'), 'simpleText'))
}

                                videosArray.push(`<li class="yt-uix-slider-slide-item ">
            
                            <div class="video-list-item  yt-tile-default ">
                              <a href="/watch?v=${video.richItemRenderer.content.videoRenderer.videoId}" class="related-video yt-uix-contextlink spf-link yt-uix-sessionlink" data-sessionlink="feature=channel&amp;ei=CLTR1ez43LQCFVKDIQodUzEL0Q%3D%3D"><span class="ux-thumb-wrap contains-addto "><span class="video-thumb ux-thumb yt-thumb-default-120 "><span class="yt-thumb-clip"><span class="yt-thumb-clip-inner"><img alt="PSY - VH1 Morning Buzz Live with Carrie and Jason" src="//i.ytimg.com/vi/${video.richItemRenderer.content.videoRenderer.videoId}/default.jpg" width="120" data-group-key="thumb-group-0"><span class="vertical-align"></span></span></span></span><span class="video-time">${videotime}</span>
 ...
 videoCountText = Panda.document.utils.findElement(Panda.document.utils.findElement(Panda.document.utils.findElement(obj, 'pageHeaderRenderer'), 'metadata'), 'metadataRows').metadataRows[1].metadataParts[1].text.content
                    ...
                    <button class="yt-uix-button yt-uix-button-default yt-uix-slider-next" rel="next"><img class="yt-uix-slider-next-arrow" src="http://web.archive.org/web/20120831215325im_///s.ytimg.com/yts/img/pixel-vfl3z5WfW.gif" alt="next"></button>
                      ...
                      <div class="yt-uix-slider-head">
                        ...
                        <div class="yt-uix-slider-title"><h2><a class="spf-link" href="/channel/${Panda.document.utils.createStringFromObject(Panda.document.utils.findElement(Panda.document.utils.findElement(obj, 'channelMetadataRenderer'), 'externalId'))}/videos">${videoCountText}
                   »</a></h2></div>
                      </div>
  ...
  run("yt.www.watch.actions.init", function() {
    var value = guid(window.location.hash);
    ...
    if (action) {
      ...
      attr(function() {
        html("watch-actions-area-container").scrollIntoView();
      }, 0);
      delete value.action;
      value = key(value) || "#!";
  ...
  run("yt.www.watch.actions.loadPlusoneLike", function(elements, dataAndEvents, deepDataAndEvents, ignoreMethodDoesntExist, textAlt) {
    new options(require(elements), dataAndEvents, deepDataAndEvents, ignoreMethodDoesntExist, textAlt, {
      size : "large",
  ...
  run("yt.www.watch.actions.report", function() {
    if (!getEnumerableProperties()) {
      /** @type {number} */
      var blue = 0;
      var youtubeObject = forEach();
      if (youtubeObject) {
        if (youtubeObject.pauseVideo) {
          youtubeObject.pauseVideo();
          blue = youtubeObject.getCurrentTime();
        }
      }
      /** @type {number} */
      blue = Math.floor(blue);
      /** @type {number} */
      youtubeObject = Math.floor(blue / 60);
      i("/watch_ajax", {
        method : "GET",
        n : {
         ...
         */
        i : function(value, obj) {
          require("report-video-content").innerHTML = obj.report_html;
          curValue.getInstance().Ic(require("report-video-content"));
          ...
          for (;pathConfig.length;) {
            parsed = pathConfig.shift();
            unbind(self, parsed.Ra, parsed.Sa, toString(parsed.H));
          }
        },
        /** @type {function (?, ?): undefined} */
        onError : setContent
...
function addClickEventListeners() {
    ...
    guideItems.forEach(guideItem => {
        guideItem.removeEventListener('click', handleClick);
        guideItem.addEventListener('click', handleClick);
    });
}
...
function handleClick() {
    ...
    const dataFunctionInfo = this.getAttribute('data-function-info');
    ...
    if (dataFunction) {
        ...
        try {
            // Split and traverse the function path
            ...
            if (typeof func === 'function') {
                func(dataFunctionInfo); // Pass dataFunctionInfo as argument
            } else {
                console.error('Function not found or not a function:', dataFunction);
            }
        } catch (error) {
            console.error('Error executing function from data-function:', dataFunction, error);
        }
    ...
    if (!this.classList.contains('guide-item-fake')) {
...
addClickEventListeners()
  ...
  run("yt.www.watch.actions.unlike", function() {
    _init();
    /** @type {boolean} */
    var killall = 1 == getDeps();
    exec(killall ? 2 : 1);
  });
  run("yt.www.watch.actions.transcript", function(inplace) {
    if (onProgress(inplace)) {
      configure();
      apply("yt.www.watch.transcript.open")(function() {
        split("watch-actions-loading");
        resize("watch-actions-transcript");
      });
    }
  });
  ...
  run("yt.www.masthead.toggleExpandedMasthead", function() {
    ...
    if (!equal(element, "loaded")) {
      ...
      /*i("/playlist_bar_ajax", {
        ...
        qb : TRUE,
        i : function(value, obj) {
          require("masthead-expanded-lists-container").innerHTML = obj.html;
